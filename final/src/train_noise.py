...
x_train_0 = np.load('x_train_log10.npy')
x_train_1 = np.load('x_train_shift_1.npy')
x_train_2 = np.load('x_train_shift_2.npy')
x_train_3 = np.load('x_train_shift_3.npy')
x_train_4 = np.load('x_train_noise_1.npy')
x_train_5 = np.load('x_train_noise_2.npy')
x_train_6 = np.load('x_train_noise_3.npy')
...
if sys.argv[1] == 'semi' or sys.argv[1] == 'all':
	...
	for loop in range(5,10):
		...
		checkpoint = ModelCheckpoint(filepath='model/model_all_'+str(loop)+'.h5', 
								 ...
								 monitor='val_acc',
								 mode='max' )
		csv_logger = CSVLogger('./log/model'+str(loop)+'.csv')

		print("\nWe add %d data into training %d %d"%(count1+count2,count1,count2))

		model.fit(x, y,validation_data=(x_valid,y_valid), epochs=1000, batch_size=10,callbacks=[checkpoint,csv_logger,earlystopping])
