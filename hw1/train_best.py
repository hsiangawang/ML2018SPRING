...
Attribute_NO = All_attri[4]
Attribute_NO2 = All_attri[5]
Attribute_NOX = All_attri[6]
#print(Attribute_PM.shape)

Attribute_PM = np.array(Attribute_PM,dtype=float)
Attribute_PM10 = np.array(Attribute_PM10,dtype=float)
Attribute_O3 = np.array(Attribute_O3,dtype=float)
Attribute_SO2 = np.array(Attribute_SO2,dtype=float)
Attribute_NO = np.array(Attribute_NO,dtype=float)
Attribute_NO2 = np.array(Attribute_NO2,dtype=float)
Attribute_NOX = np.array(Attribute_NOX,dtype=float)
...
for i in range(Attribute_O3.shape[0]):
    if Attribute_O3[i] <= 0 :
        Attribute_O3[i] = (Attribute_O3[i-1] + Attribute_O3[i+1])/2
for i in range(Attribute_NO.shape[0]):
    if Attribute_NO[i] <= 0 :
        Attribute_NO[i] = (Attribute_NO[i-1] + Attribute_NO[i+1])/2
for i in range(Attribute_NO2.shape[0]):
    if Attribute_NO2[i] <= 0 :
        Attribute_NO2[i] = (Attribute_NO2[i-1] + Attribute_NO2[i+1])/2
for i in range(Attribute_NOX.shape[0]):
    if Attribute_NOX[i] <= 0 :
        Attribute_NOX[i] = (Attribute_NOX[i-1] + Attribute_NOX[i+1])/2
...
Attribute_O3 = np.append(Attribute_O3[ :4800] , Attribute_O3[5280:])
Attribute_NO = np.append(Attribute_NO[ :4800] , Attribute_NO[5280:])
Attribute_NO2 = np.append(Attribute_NO2[ :4800] , Attribute_NO2[5280:])
Attribute_NOX = np.append(Attribute_NOX[ :4800] , Attribute_NOX[5280:])

month = All_attri[ : , 4800 : 5280 ]
month = np.array(month,dtype=float)
...
for i in range(Attribute_PM.shape[0]-480-9):
    ...
    fea_O3 = np.array(Attribute_O3[i:i+9])
    fea_SO2 = np.array(Attribute_SO2[i+4:i+9])
    fea_NO = np.array(Attribute_NO[i:i+9])
    fea_NO2 = np.array(Attribute_NO2[i:i+9])
    fea_NOX = np.array(Attribute_NOX[i:i+9])

    new_feature = np.append(fea_2,fea_10)
    new_feature = np.append(new_feature,fea_O3)
    new_feature = np.append(new_feature,fea_NO)
    new_feature = np.append(new_feature,fea_NO2)
    new_feature = np.append(new_feature,fea_NOX)
...
for i in range(471):
    ...
    O3 = np.array(month[7, i:i+9])
    SO2 = np.array(month[12, i+4:i+9])
    NO = np.array(month[4 ,i:i+9])
    NO2 = np.array(month[5 ,i:i+9])
    NOX = np.array(month[6 ,i:i+9])

    #new_feature = np.append(PM2,PM2)
    new_feature = np.append(PM2,PM10)
    new_feature = np.append(new_feature,O3)
    new_feature = np.append(new_feature,NO)
    new_feature = np.append(new_feature,NO2)
    new_feature = np.append(new_feature,NOX)
